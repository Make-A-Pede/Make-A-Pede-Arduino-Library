/**
 * ColorDemoAntennae.ino - Software library for the Make-A-Pede (makeapede.com)
...
/*
 * Avoid obstacles using the antennae
 * count is the number of obstacles to avoid
 */
void obstacleAvoidAntenna(int count){
  int objectCounter = 0;
  while(objectCounter < count) {
    ...
    if (OBSTACLE_LEFT) {
      setLeftSpeed(defaultSpeed/2);
      setRightSpeed(defaultSpeed);
    } else if (OBSTACLE_RIGHT) {
      setLeftSpeed(defaultSpeed);
      setRightSpeed(defaultSpeed/2);
    }
